...
use App\Models\DeliveryMethod; use App\Models\Order;
use App\Services\Cart\CartService; use App\Services\Orders\OrderService;
...
use Illuminate\Validation\Rule; use Illuminate\View\View;
class CheckoutController extends Controller {
    public function __construct(protected CartService $cart, protected OrderService $orders, protected YooKassaService $yookassa) {}
    public function index(): View|RedirectResponse {
        if ($this->cart->count() === 0) return redirect()->route('cart.index');
        $tenant = app(TenantContext::class)->current();
        return view('shop.checkout', [
            'items' => $this->cart->items(), 'total' => $this->cart->total(),
            'deliveryMethods' => DeliveryMethod::query()->active()->get(),
    ...
    public function store(Request $request): RedirectResponse {
        ...
        if ($minOrder !== null && (float)$minOrder > 0 && $this->cart->total() < (float)$minOrder) {
            return redirect()->route('cart.index')->withErrors(['cart' => 'Минимальная сумма заказа — '.number_format((float)$minOrder,0,',',' ').' ₽.']);
        }
        $activeIds = DeliveryMethod::query()->active()->pluck('id')->all();
        $rules = [
            'name'=>['required','string','max:255'],'phone'=>['required','string','max:32'],
            'email'=>['nullable','email','max:255'],'delivery_address'=>['nullable','string','max:1000'],
            'payment_method'=>['required',Rule::in(['cash','card_online','card_courier'])],
            'comment'=>['nullable','string','max:2000'],
        ];
        if ($activeIds !== []) $rules['delivery_method_id'] = ['required', Rule::in($activeIds)];
        else $rules['delivery_method'] = ['required', Rule::in(['pickup','courier','post'])];
