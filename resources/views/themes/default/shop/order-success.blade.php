...
@section('content')
    <div class="mx-auto max-w-lg text-center">
        <div class="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-full bg-emerald-100 text-3xl text-emerald-700">✓</div>
        ...
        <p class="mt-1 text-2xl font-extrabold text-primary">{{ number_format((float) $order->total, 0, ',', ' ') }} ₽</p>
        ...
        <div class="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
            <a href="{{ route('order.track', ['number' => $order->number]) }}"
               class="rounded-xl bg-primary px-6 py-3 font-semibold text-white hover:opacity-90">
                Следить за статусом
            </a>
        ...
        <ul class="mt-10 space-y-2 rounded-2xl bg-white p-5 text-left text-sm shadow-sm ring-1 ring-slate-200/60">
            @foreach ($order->items as $item)
                <li class="flex justify-between gap-3 border-b border-slate-50 pb-2 last:border-0 last:pb-0">
                    <span>{{ $item->product_name }} × {{ $item->quantity }}</span>
                    <span class="font-medium">{{ number_format((float) $item->total, 0, ',', ' ') }} ₽</span>
                </li>
            @endforeach
        </ul>
    </div>
