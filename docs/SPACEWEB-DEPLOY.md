...
>
> ⚠️ В панели SpaceWeb выберите для сайта **PHP 8.2 или 8.3** (раздел «Сайты →
> Управление сайтом → Версия PHP»): CLI-php по умолчанию может быть старым,
> из-за чего artisan даёт `Parse error`. Workflow сам ищет бинарники
> `php8.3/php8.2/php8.1/php8.0` на сервере для миграций, а для самого сайта
> версия задаётся в панели.
...
> Если весь проект загружается прямо в `public_html` (как делает CI-деплой из
> раздела 0), в корне проекта лежит готовый [`.htaccess`](../.htaccess): он
> перенаправляет все запросы в `public/` и закрывает прямой доступ к служебным
> файлам и каталогам (`/.env`, `storage/`, `vendor/` и т.п.).
